...
\texit{Smart ITS Face Recognition System} (Siffars) merupakan sistem pendeteksi wajah yang menggunakan \texit{facenet} sebagai basis model pendeteksi wajah.
Input yang digunakan dalam Siffars adalah video yang berasal dari cctv ataupun \texit{webcam} yang bersifat \texit{real-time} maupun tidak. 
Sedangkan input model \texit{facenet} yang digunakan adalah foto wajah yang telah di- \texit{crop} menggunakan \texit{face detector} dan di- \texit{encoding} menggunakan base64.
Model \texit{facenet} akan menghasilkan \texit{vector embeddings} sepanjang 512 yang menginterpretasikan wajah yang telah terdeteksi.
...
\begin{figure} [ht] \centering
  ...
  \caption{Arsitektur \texit{Smart ITS Face Recognition System}}
...
Pada KP ini digunakan perangkat \texit{personal computer} sebagai berikut:
\begin{itemize}
...
Teknik Komputer ITS. Menggunakan CCTV/ \texit{webcam} sebagai input videonya, Siffars dapat mendeteksi kehadiran untuk
setiap pegawai berdasarkan kemiripan \texit{vector embeddings}. Sebelumnya, data wajah setiap pegawai harus
disimpan dalam \texit{database} terlebih dahulu untuk dibandingkan dengan \texit{vector embeddings} ketika Siffars dijalankan.

Proses instalasi Siffars memerlukan beberapa hal dan \texit{depedency} agar Siffars dapat berjalan dengan optimal.

\subsection{Instalasi CCTV}
\begin{itemize}
  \item Sambungkan CCTV (Gambar \ref{fig:cctv}) dengan kabel LAN(RJ45) (Gambar \ref{fig:kabellan}) dan \texit{adaptor} (Gambar \ref{fig:adapter}) sebagai \texit{power supply}nya
  \item Sambungkan \texit{Switch} (Gambar \ref{fig:switch}) dengan \texit{adaptor} (Gambar \ref{fig:adapter}) sebagai \texit{power supply} nya
  \item Sambungkan NVR (Gambar \ref{fig:nvr}) dengan \texit{adaptor} (Gambar \ref{fig:adapter}) sebagai \texit{power supply}, dan sambungkan ke LCD/Monitor untuk mendapatkan tampilan CCTV
  \item Sambungkan kabel LAN (Gambar \ref{fig:kabellan}) yang berasal dari CCTV (Gambar \ref{fig:cctv}) ke \texit{Switch} (Gambar \ref{fig:switch})
  \item Sambungkan kabel LAN (Gambar \ref{fig:kabellan}) yang berasal dari \texit{switch} (Gambar \ref{fig:switch}) ke NVR (Gambar \ref{fig:nvr})
...
OpenCV yang di\texit{compile} dengan \texit{support} CUDA dibutuhkan untuk menjalankan SIFARS.
...
\begin{itemize}
  ...
  Perlu diperhatikan CUDA ARCH BIN dalam \texit{script} diatas, disesuaikan dengan \texit{compute capability} masing-masing GPU, dalam KP ini menggunakan GPU NVIDIA GTX 1650.

  \item Jika diperhatikan \texit{output} dari \texit{command} di atas, pada bagian akhir akan terlihat seperti berikut:
  ...
  \item Bila bagian CUDA dan Python \texit{interpreter} yang dimaksud sudah sesuai, artinya konfigurasi sudah benar
  \item Lakukan proses \texit{build} dengan
  
  \begin{lstlisting}
    make -j$(nproc)
  \end{lstlisting}

  \item Setelah proses selesai, lakukan \texit{install} dengan:
  
  \begin{lstlisting}
    sudo make install
  \end{lstlisting}

  \item Lakukan \texit{symlinking} ke direktori \texit{environtment variable} conda dengan:
  
  \begin{lstlisting}
    ln -s /usr/local/lib/python3.6/site-packages/cv2 ~/miniconda3/envs/sf/lib/python3.6/site-packages/cv2
  \end{lstlisting}

  \item Cek apakah OpenCV sudah ter\texit{install} dengan:
  
  \begin{lstlisting}
    echo $(python -c "import cv2; print('Version: ', cv2.__version__);print('\n', cv2.getBuildInformation());")
  \end{lstlisting}

  dan pastikan \texit{output} memberikan versi yang sama dengan versi OpenCV yang di\texit{download} tadi, dan pastikan terdapat baris yang menyatakan: NVIDIA CUDA: YES (ver 10.2 ...)

\end{itemize}

\subsection{Instalasi Siffars}
\textbf{\texit{Requirements} dan \texit{setup}}

\begin{itemize}
  \item CUDA Toolkit 10.x (telah dites pada versi 10.0, 10.1, dan 10.2) dan cuDNN (versi disesuaikan CUDA Toolkit yang ter\texit{install})
  \item Install \href{https://docs.docker.com/engine/install/ubuntu/}{Docker for Linux}
  \item \texit{Add user} linux ke group docker untuk eksekusi tanpa sudo: sudo adduser "username" docker lalu \texit{reboot}
  \item (Opsional) Install Miniconda3 lalu buat sebuah conda \texit{environment} dengan nama misal sf dengan versi Python 3.6:
  ...
  NB: Conda bersifat opsional, boleh menggunakan \texit{virtualenv} atau \texit{environment manager} Python lain

  \item Setelah aktivasi \texit{environment}, \texit{install} Numpy dengan
  \begin{lstlisting}
    pip install numpy
  \end{lstlisting}  

  \item \texit{Install} semua \texit{dependency} dengan:
  ...
  Create env untuk \texit{facenet} (hanya pertama kali):
  \begin{lstlisting}
  ...
  \end{lstlisting}
  Clone dan run \texit{facenet} dengan perintah:
  \begin{lstlisting}
...
\begin{enumerate}
\item Menjalankan seluruh aplikasi dan \texit{database} yang digunakan untuk Siffars
\item \texit{Admin} membuka halaman \texit{web} untuk \texit{register/login} jika sudah pernah mendaftar sebelumnya
\item \texit{Admin} memilih menu "\texit{add person}" untuk menambahkan data wajah pegawai ke dalam \texit{database} Siffars (semakin banyak data wajah semakin akurat)
\item Sistem akan otomatis mendeteksi wajah pegawai yang telah ditambahkan
dan nantinya akan ditampilkan ke \texit{web} saat pegawai tersebut terdeteksi.
\end{enumerate}
...
\begin{figure} [p] \centering
  ...
  % Keterangan gambar yang diinputkan
  \caption{\texit{Page admin} untuk melakukan registrasi}
  % Label referensi dari gambar yang diinputkan
  \label{fig:SfRegis}
  
  \includegraphics[scale=0.2]{gambar/login.png}
  \caption{\texit{Page admin} untuk melakukan \texit{login} (jika sudah pernah mendaftar sebelumnya)}
  \label{fig:SfLogin}
\end{figure}

\begin{figure} [p] \centering
  \includegraphics[scale=0.2]{gambar/addpersonfix.png}
  \caption{\texit{Page admin} untuk menambah data wajah pegawai}
  \label{fig:SfAddperson}

  \includegraphics[scale=0.2]{gambar/listperson.png}
  \caption{\texit{Page admin} untuk melihat data wajah pegawai}
  \label{fig:SfListperson}

  \includegraphics[scale=0.2]{gambar/listface.png}
  \caption{\texit{Page admin} untuk melihat data wajah tiap pegawai}
  \label{fig:SfListperson}
\end{figure}

\begin{figure} [p] \centering
  \includegraphics[scale=0.2]{gambar/listuser.png}
  \caption{\texit{Page admin} untuk melihat \texit{data user} yang bisa mengakses \texit{web} Siffars}
  \label{fig:SfListperson}

  \includegraphics[scale=0.2]{gambar/editaccount.png}
  \caption{\texit{Page admin} untuk mengedit akun}
  \label{fig:SfListperson}
...
\begin{figure} [p] \centering  
  ...
  \caption{\texit{Adaptor} untuk \texit{switch}, nvr, dan CCTV}
  \label{fig:adapter}
